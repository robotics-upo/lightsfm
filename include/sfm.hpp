...
struct Agent
{
  ...
  Agent(const utils::Vector2d& position, const utils::Angle& yaw, double linearVelocity,
...
inline void SocialForceModel::computeSocialForce(unsigned index, std::vector<Agent>& agents) const
{
  ...
  for (unsigned i = 0; i < agents.size(); i++)
  {
    ...
    double forceAngleAmount =
        -theta.sign() * std::exp(-diff.norm() / B - PW(agent.params.n * B * thetaRad));
...
inline std::vector<Agent>& SocialForceModel::updatePosition(std::vector<Agent>& agents,
                                                            double dt) const
{
  for (unsigned i = 0; i < agents.size(); i++)
  {
    ...
    if (agents[i].teleoperated)
    {
      ...
      utils::Vector2d inc(
          imd * std::cos(agents[i].yaw.toRadian() + agents[i].angularVelocity * dt * 0.5),
